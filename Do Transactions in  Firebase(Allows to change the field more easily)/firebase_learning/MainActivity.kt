...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?){
        ...
        binding.btnBatchedWrite.setOnClickListener {
            // Note that I am hardcoring the id of one of the documents in my firebase console
            changeName(
                "S7R6j826m0OJn7H9vGeP","Adebayo","Alade")

        }
        binding.btnTransaction.setOnClickListener {
            birthday(
                    "S7R6j826m0OJn7H9vGeP")
    ...
    // This is a runTransaction which allows you to able to access a document and then change it directly
    private fun birthday(personId: String) = CoroutineScope(Dispatchers.IO).launch {
        try{
            Firebase.firestore.runTransaction { transaction ->
                ...
                val person = transaction.get(personRef) // Note that "person" here then becomes a document snapshot
                val newAge = person["age"] as Long +1 // Note that we have to cast it as a long just to declare the data type, here we are changing the value of the age directly
    ...
    // To create a function that is used to change the first and last name using batched write
    private fun changeName(
        ...
        try {
            Firebase.firestore.runBatch { batch -> // Note that returns a Write batch usually
                // To get the id of the person document
                val personRef = personCollectionRef.document(personId)
                batch.update(personRef,"firstName",newFirstName)
                // batch.delete() // We can also delete using this batch
                //batch.set() // We can also set
                batch.update(personRef,"lastName",newLastName)
                // Note the difference
                // Using this batch only works when everything works fine and it wont work if any one of them fails
                // Note that in a run Batch then it has to do both of them alone
            }.await()
    ...
    private fun retrievePersons() = CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            // Queryshop is the real document not the reference
